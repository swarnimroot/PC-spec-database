...
| T7.3 | Update `SESSION_LOG.md` with implementation milestones as they land | Partial — Sessions 13, 14, 15, 17 entries cover T7.0a–c + T7.2 closeouts; final stage-close milestone follows T7.0d. |
| T7.4 | Refresh-by-stored-source_url CLI subcommand (or `refresh --from-db` flag) — read a product's stored `source_url` from `products.<bundle>` provenance and use that for the live fetch, bypassing the hard-coded `DEFAULT_*_URL_TMPL` constants in `cli/refresh.py`. Removes the "remember the full marketing slug" trap surfaced by Session 17 smoke; first-time ingests still use templates. | New (Session 17 finding). |

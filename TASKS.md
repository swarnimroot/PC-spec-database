...
**Status (Session 14, 2026-05-08):** T7.0c shipped (boards.label view-layer ordinal renumbering with tier-ascending sort; bridge tier labels intact; 192 → 198 tests). T7.0b shipped Session 13 (7 actionable bridge + CLI fixes; 172 → 192 tests). T7.0a (Lenovo merge ingest) and T7.0d (keyboard structured offerings) still open; resolution_label folds into T7.1. Deferrals: Finding #1 HP upstream-blocked; Finding #3 Lenovo www→psref redirect rolled into T7.0a.
...
| T7.0c | Boards.label per-product ordinal renumbering — view-layer rewrite (preserves bridge tier-merge semantics). Decoded: bridge keeps internal MB1/MB2/MB3 tier labels for cross-tile merge; view layer renames them to per-product ordinals at render time so an ac16251 with no top-tier SKU shows MB1 + MB2 instead of MB2 + MB3. | Done — Session 14. `views/boards.py:render` sorts offerings by tier (MB1 < MB2 < MB3, unmapped last) before assigning per-product ordinals; bridge / merge / `field_paths` unchanged. New `tests/views/test_boards.py` (+6 tests; 192 → 198). Caveat: `boards.{idx}.label` still surfaces in `manual-edit` field paths — edits are silently masked at render. |

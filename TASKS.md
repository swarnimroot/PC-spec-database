...
**Status (Session 18, 2026-05-11):** T7.4 shipped (`refresh --from-db` flag + `--year` disambiguator in `cli/refresh.py`; new `_collect_source_urls_from_product` + `_walk_source_urls` helpers walk the product row's JSON columns and collect distinct scraped `source_url` values; mutex with `--url`; manual-only products error with helpful message; Lenovo Intel+AMD multi-URL merge handled by looping fetch across each distinct stored URL; 11 new tests; 242 → 253). T7.2 shipped Session 17 (end-to-end smoke — all 6 products refreshed successfully across Dell / HP / Lenovo / ASUS bridges; T7.0a family_code merge confirmed end-to-end on live PSREF; live drift captured on Dell aa18250 `storage_slots` + `keyboard_offerings`; 15 → 23 unresolved queue rows from drift + year_inferred re-fires; 242/242 tests unchanged at that point). The Session 17 initial smoke claimed 4 scrapers-lib parser regressions; user pushed back, investigation showed those were CLI URL template mismatches (templates didn't emit the same URLs as DB-stored provenance) — filed as T7.4 and closed this session. T7.1 shipped Session 16 (docs polish — README Setup + Upgrading + Normalization notes + per-CLI CLI reference for all 9 subcommands; DATA_MODEL.md `resolution_label` cross-link). T7.0a shipped Session 15 (Lenovo multi-URL merge ingest — schema + slug parser + bridge + grouping coercion + runner premerge + backfill CLI + plain-leaf manual-edit shape; 198 → 242 tests across 7 commits). T7.0c shipped Session 14 (boards.label view-layer ordinal renumbering; 192 → 198 tests). T7.0e shipped Session 14 close. T7.0b shipped Session 13 (7 actionable bridge + CLI fixes; 172 → 192 tests). **T7.0d (keyboard structured offerings) is the largest open Stage 7 task**; T7.3 still rolling.
...
| T7.2 | End-to-end smoke test across all four vendors | Done — Session 17. Live refresh of all 6 products across Dell / HP / Lenovo / ASUS bridges, all succeeded end-to-end (Dell aa18250 + ac16251, HP 16t-ah100, Lenovo `Legion_Pro_7_16AFR10H` with T7.0a family_code merge confirmed live, ASUS Zephyrus G16, ASUS Strix G16). Initial run failed 4 of 6 with parser errors which I incorrectly attributed to scrapers-lib regressions; user caught it; investigation showed the failures were CLI URL template mismatches (the `--model` slug templates emit different URLs than what was originally ingested for Dell Aurora line, HP full-slug PDP, and ASUS Strix). Re-running with `--url` overrides from DB-stored provenance succeeded for all 4. Real finding: URL template gap, filed as T7.4. `find-conflicts` verified detection (15 → 23 unresolved; +8 from live drift — Dell aa18250 `storage_slots` 2→1 slot + `keyboard_offerings` tier consolidation as `value_disagreement`, Lenovo lighting + camera `value_disagreement`, plus 4 `year_inferred` re-fires). `audit-normalize` surfaced 48 paths; 2 real normalization observations captured (`resolution_label` vendor-preserved per T7.1 enum-pair; `camera_offerings.0.resolution` unit mismatch on Lenovo) — both deferred, not scoped into T7.2. Pipeline declared healthy. Process lesson saved as `feedback_url_template_check.md`. |
| T7.3 | Update `SESSION_LOG.md` with implementation milestones as they land | Partial — Sessions 13, 14, 15, 17, 18 entries cover T7.0a–c + T7.2 + T7.4 closeouts; final stage-close milestone follows T7.0d. |
| T7.4 | Refresh-by-stored-source_url CLI subcommand (or `refresh --from-db` flag) — read a product's stored `source_url` from `products.<bundle>` provenance and use that for the live fetch, bypassing the hard-coded `DEFAULT_*_URL_TMPL` constants in `cli/refresh.py`. Removes the "remember the full marketing slug" trap surfaced by Session 17 smoke; first-time ingests still use templates. | Done — Session 18. `cli/refresh.py` gains `--from-db` + `--year` flags. New helpers: `_collect_source_urls_from_product` (DB lookup by `(model_code, year)`, walks every JSON-encoded column on the row, plain-TEXT columns fall through `json.JSONDecodeError`, dedupes preserving order) + `_walk_source_urls` (pure recursive walker; appends every non-empty `source_url` string from scalar bundles and offering-list bundles). Mutex with `--url`; manual-only rows error with helpful message; Lenovo Intel+AMD multi-URL merge handled by looping fetch across each distinct stored URL (snapshots accumulate, downstream PK-grouping + ingest unchanged). 11 new tests in `tests/cli/test_refresh.py` (5 walker + 6 DB collector incl. Lenovo multi-URL pattern, manual-only, year disambiguation, missing product). 242 → 253 tests. Live smoke verified Session 18 close: `refresh --brand dell --model aa18250 --from-db` ran clean fetch-to-ingest — 3 snapshots, inserted=0 refreshed=48 conflicts=2 low_confidence=0 year_inferred=1; the 2 conflicts match Session 17's known drift (`storage_slots` + `keyboard_offerings`) and are untouched by T7.4. |
...
| `camera_offerings.0.resolution` unit mismatch | Lenovo stores sensor MP (`5.0MP` / `1440p`), other vendors store video resolution (`1080p`). Surfaced by Session 17 `audit-normalize`. Future camera-leaf normalization conversation, not scoped into T7.2. |

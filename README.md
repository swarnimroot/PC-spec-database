...
Landing page is the **hub** — four health metrics (products / vendors / queue / days since last refresh) over two destination rows:

- **Explore** (read-only): **Browse one product**, **Compare side-by-side**, **Find products where…**
- **Curate** (writes the DB): **Review queue triage**, **Manual-edit a cell**, **Refresh products**

The three Curate screens write through the same library handlers as the `resolve`, `manual-edit`, and `refresh` CLIs — so a UI write and a CLI write produce identical bundles + provenance. See the [`ui` subcommand reference](#ui) below for flags.
...
Stage 8 / Phase 2 complete (Session 35): hub (Explore + Curate rows + four health metrics), browse, compare, find, review-queue triage, manual-edit-a-cell, and refresh-trigger screens are live (T8.0–T8.7). T8.8 polish shipped Sessions 34–35: README "Launching the UI" Quick-start subsection + Custom-URL host pre-flight check (`_validate_url_brand`) + `streamlit.testing.v1.AppTest` harness under `tests/ui/` covering entry-point boot + view dispatcher + one read-only screen smoke + all 3 write screens' write-path interaction tests + the refresh error pipeline.

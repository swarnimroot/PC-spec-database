...
For the browser UI, see [§Launching the React frontend + FastAPI backend](#launching-the-react-frontend--fastapi-backend) below.
...
### Launching the React frontend + FastAPI backend

The UI is a **React frontend + FastAPI backend** over the unchanged Python/SQLite core (see [`REACT_REBUILD_PLAN.md`](docs/REACT_REBUILD_PLAN.md)). As of Session 55 it is the **sole UI** — the legacy Streamlit app was retired in the Phase 4 cutover.

- **Backend** — `competitive_database/api/` (FastAPI; `fastapi` + `uvicorn` are in core `dependencies`). Wraps the importable data/query layer with **no Streamlit dependency**. Endpoints: `GET /api/health`, `/api/schema`, `/api/catalog`, `/api/model/{id}?year=`, `/api/model/{id}/detail?year=` (raw per-leaf detail for the Compare accordion — Session 58), `/api/schema/detail` (Compare field-visibility panel — Session 58), `POST /api/find`, `GET /api/queue`, `/api/queue/counts`, `POST /api/value`, `/api/resolve`, `GET /api/value/history`, `POST /api/refresh`, and the Add-product trio (Session 61): `GET /api/vendors` (one entry per brand `{brand, label, links:[{label, url}]}` — ASUS carries two links, ROG + ASUS, both `brand="asus"`), `POST /api/products/scrape` (`{brand, url}` → `{token, products:[{model_code, year, name, brand, summary, already_exists}]}` — one live fetch, no DB write, `name` is the cleaned editable default), `POST /api/products/add` (`{token, names?}` → additive ingest that skips existing PKs so it never queues conflicts; `names` keyed by `"<model_code>|<year>"` writes `products.product`). A "model" = one Brand·Series·Product identity spanning its years (37 models; id = latest year's model_code; base + per-year `byYear` overrides). Values serialize to `{v, s, src, ts, note}`; the 6 stored statuses map to 5 display states (verified+vouched→confirmed, vendor-doesn't-publish→not_published, manual→hand, needs-review→review, empty→blank). DB path from `COMPETITIVE_DB_PATH` (falls back to `competitive.db`). CORS allows localhost dev origins (3000 / 5173 / 8501).
- **Frontend** — `frontend/` (Vite + React, plain JSX). Shared lib `src/shared/` (`StateDot`, `ValueCell`, data helpers). Five screens shipped and verified on the real DB: **Home** (`src/home/` — hero + live counts (Products / Vendors / Conflicts / Unverified from `/api/catalog` + `/api/queue/counts`) + clickable stat cards (Products/Vendors → Spec Finder; Conflicts/Unverified → Review) + clickable screen cards + a per-session welcome modal persisted in sessionStorage; the default screen, and the brandmark routes here), **Spec Finder** (`src/finder/` — faceted filters with live counts + an Advanced field+operator query preserving `is_empty` / `vendor_unavailable` / numeric ops), **Compare Matrix** (`src/compare/` — rows = our sections, ≤4 columns via drag-shelf + typeahead, per-column year scrubber, differences-only; only the Processor / Graphics / I/O sections are collapsible accordions that expand to real per-leaf values via `/api/model/{id}/detail` (multi-offering leaves stacked on separate lines), every other section a plain row, plus a ☰ Fields drawer to toggle fields/sections — Session 58), **Review** (`src/curation/` — unified queue with **Conflicts / Unverified / Missing / Manual** buckets grouped by spec category in importance order, adaptive editor with a Conflicts "Correct value" box routing to `manual_override`, keyboard commit, responsive provenance slide-over; replaces Edit + Triage — grouping/edit-box Session 57), and **Add/Refresh** (`src/refresh/`; nav tab renamed "Refresh" → "Add/Refresh" in Session 61, route id unchanged — "All eligible products" or "One product" modes over `POST /api/refresh`, a Run-refresh confirmation dialog (modal, Session 61), running spinner, results rollup with conflict callout to Review; plus a **"+ Add new"** Add-product flow — pick vendor → vendor landing links → paste product URL → one live scrape → preview-then-confirm with an editable product-name field → additive save (`AddProduct.jsx`, Session 61)). Top nav: **Home | Spec Finder | Compare | Review | Add/Refresh**. The browser calls the API **same-origin** under the base path (`/competitive-database/api/...`), which the Vite dev server proxies to the backend — no host is hardcoded in the bundle, so the app works on localhost and behind a reverse proxy / Tailscale Funnel alike.

**One-command launch** (runs both halves via `concurrently` + `cross-env`):
...
This opens the app at **http://localhost:8501/competitive-database/** (Vite `base` `/competitive-database/` + `strictPort` 8501) backed by uvicorn on **:8011**, against the real `competitive.db`. The browser calls the API **same-origin** under the base path (`/competitive-database/api/...`); the Vite dev server proxies `/competitive-database/api` → `http://localhost:8011` (target overridable via `VITE_API_TARGET`). Because no host is baked into the bundle, the same build is reachable over a reverse proxy — e.g. the existing **Tailscale Funnel** mapping (`/competitive-database` → `localhost:8501/competitive-database/`) exposes the app publicly with no code change. `frontend/vite.config.js` allows `.ts.net` hosts so the Funnel host can load the dev server. `VITE_API_BASE` in `frontend/.env` is an optional override (left unset so same-origin is the default).

The React rebuild is complete: the Refresh screen (Session 55), the Home + welcome modal (Session 55), and the Phase 4 cutover (Session 55 — the Streamlit `ui/`, `ui-launch` CLI, and `tests/ui/` deleted; `ui` optional dependency dropped) are all **DONE**. The React frontend is the sole UI. See [`REACT_REBUILD_PLAN.md`](docs/REACT_REBUILD_PLAN.md).
...
### Code shape (two-tier, as of Session 54)

- **Python core** (unchanged): `competitive_database/db/`, `bridge/`, `ingest/`, `views/`, `cli/` (10 CLIs).
- **Query engine** — `competitive_database/query/`: pure, streamlit-free criteria matching extracted from `ui/find.py` (operators `eq` / `gte` / `lte` / `contains` / `is_set` / `is_empty` / `vendor_unavailable`).
- **Backend** — `competitive_database/api/`: FastAPI over the importable data/query layer (no Streamlit dependency). Plus `competitive_database/curation/` — streamlit-free conflict-queue listing extracted from `ui/triage.py`.
- **Frontend** — `frontend/`: Vite + React (plain JSX). Home, Spec Finder, Compare Matrix, Review, Refresh.

The Streamlit `competitive_database/ui/` package was retired in the Session 55 Phase 4 cutover — the React frontend is the sole UI.

Detailed system design: see [`ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
- **ASUS** — ROG spec pages must include the `/spec/` subpath, e.g. `https://rog.asus.com/laptops/.../<model>/spec/`; TUF and V16 spec pages live on `www.asus.com` under `for-gaming/...` with a trailing `/techspec/`. The bare landing page returns no spec sections. In template mode (`--model` slug), the slug prefix picks the right template via `ASUS_SERIES_TEMPLATES` in `cli/refresh.py` (Session 62).
...
Under the Stage 11 PK swap (Session 48), `MODEL_CODE` positional / `--model` flag callsites still pass the vendor slug; each CLI resolves it to the `(product, year)` PK via `resolve_products_pk` in `db/helpers.py` (Stage 11 Phase 1.5, Session 62 — replaced the `_normalize_products_pk` compat shim), so every pre-Stage-11 CLI signature continues to work unchanged.
...
> The browser UI is no longer a CLI subcommand. The legacy `ui` (Streamlit) launcher was retired in the Session 55 Phase 4 cutover; the UI is now the React frontend + FastAPI backend (see [§Launching the React frontend + FastAPI backend](#launching-the-react-frontend--fastapi-backend)).

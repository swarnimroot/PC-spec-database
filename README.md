...
**Current (Session 54, 2026-06-03):** The project is mid-pivot from a Streamlit UI to a **React frontend + FastAPI backend**, keeping the Python/SQLite core unchanged. New modules: `competitive_database/query/` (pure, streamlit-free query engine extracted from `ui/find.py`), `competitive_database/api/` (FastAPI backend), `competitive_database/curation/` (streamlit-free conflict-queue listing), and `frontend/` (Vite + React). Three React screens are shipped and verified on the real DB — **Spec Finder**, **Compare Matrix**, and **Curation Cockpit** — launched with one command (`cd frontend && npm run dev` → http://localhost:8501/competitive-database/, backend on :8011). The legacy Streamlit app (`competitive_database/ui/`) still exists and works — it is **not yet retired** (React parity is partial). Not yet done: the React Refresh screen, a lightened Home + once-ever welcome modal, and the Phase 4 cutover. **561 tests pass.** See [`REACT_REBUILD_PLAN.md`](docs/REACT_REBUILD_PLAN.md) and `SESSION_LOG.md` Session 54. The detailed stage history below is retained as the historical record.
...
### Launching the Streamlit UI (legacy — being superseded by React)

> **Note:** The Streamlit UI is now **legacy**. The current primary UI is the React frontend + FastAPI backend (see [§Launching the React frontend + FastAPI backend](#launching-the-react-frontend--fastapi-backend-current-primary-ui) below). Streamlit is **not yet retired** — it still works and React parity is partial — but new UI work is happening in React.
...
### Launching the React frontend + FastAPI backend (current primary UI)

As of Session 54 the UI is moving to a **React frontend + FastAPI backend** over the unchanged Python/SQLite core (see [`REACT_REBUILD_PLAN.md`](docs/REACT_REBUILD_PLAN.md)). The Streamlit app above is now **legacy** — still present and working, but being superseded; React parity is partial, so both exist for now.
...
- **Frontend** — `frontend/` (Vite + React, plain JSX). Shared lib `src/shared/` (`StateDot`, `ValueCell`, data helpers). Three screens shipped and verified on the real DB: **Spec Finder** (`src/finder/` — faceted filters with live counts + an Advanced field+operator query preserving `is_empty` / `vendor_unavailable` / numeric ops), **Compare Matrix** (`src/compare/` — rows = our sections, ≤4 columns via drag-shelf + typeahead, per-column year scrubber, differences-only), and **Curation Cockpit** (`src/curation/` — unified conflict + field-state queue, adaptive editor, keyboard commit, responsive provenance slide-over; replaces Edit + Triage). Top nav: **Spec Finder | Compare | Curation**.
...
This opens the app at **http://localhost:8501/competitive-database/** (Vite `base` `/competitive-database/` + `strictPort` 8501) backed by uvicorn on **:8011**, against the real `competitive.db`. The API base URL is configurable via `frontend/.env` (`VITE_API_BASE`, default `http://localhost:8011`).

Not yet done in the React rebuild: the Refresh screen, the lightened Home + once-ever welcome modal, and the Phase 4 cutover (retiring the Streamlit `ui/`). See [`REACT_REBUILD_PLAN.md`](docs/REACT_REBUILD_PLAN.md).
...
- **Frontend** — `frontend/`: Vite + React (plain JSX). Spec Finder, Compare Matrix, Curation Cockpit.
- **Legacy** — `competitive_database/ui/`: the Streamlit app, still present and working, being superseded by the React frontend (not yet retired).

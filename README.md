...
**Current (Session 55, 2026-06-09):** The pivot from Streamlit to a **React frontend + FastAPI backend** is complete — the Streamlit UI has been **retired** (Phase 4 cutover). The Python/SQLite core is unchanged. Modules: `competitive_database/query/` (pure, streamlit-free query engine extracted from the old `ui/find.py`), `competitive_database/api/` (FastAPI backend), `competitive_database/curation/` (streamlit-free conflict-queue listing), and `frontend/` (Vite + React). Five React screens are shipped and verified on the real DB — **Home**, **Spec Finder**, **Compare Matrix**, **Review**, and **Refresh** — launched with one command (`cd frontend && npm run dev` → http://localhost:8501/competitive-database/, backend on :8011). The React frontend is now the **sole UI**; the Streamlit `competitive_database/ui/` package, the `ui-launch` CLI, and `tests/ui/` were deleted, and the `ui = ["streamlit>=1.40"]` optional dependency was dropped from `pyproject.toml`. The `refresh` CLI and `POST /api/refresh` remain as the non-UI refresh path. **515 tests pass.** See [`REACT_REBUILD_PLAN.md`](docs/REACT_REBUILD_PLAN.md) and `SESSION_LOG.md` Sessions 54–60. The detailed stage history below is retained as the historical record.

...
**Stage 10b (CPU + Graphics rollup) — closed Session 44 (2026-05-20).** Browse / Compare / Find collapse the CPU and Graphics sections to a single rollup row each; per-SKU detail is preserved in the DB and stays visible on the Edit screen. CPU rollup deduplicates `architecture_code` values across `cpu_offerings` (comma-joined; e.g. `RPL-H R` / `HWK R`). Graphics rollup walks each board's `gpus`: NVIDIA contributes the catalog `board` value (e.g. `MB1` / `MB2` / `MB3`), AMD/Intel contribute the brand name, `gpu_class = "integrated"` rows drop out entirely. Both rollups carry one worst-status marker across per-SKU offerings (`needs-review > vendor-doesn't-publish > manual > verified`). Schema additions (idempotent `ALTER` migrations): `cpu_catalog` — renamed empty `architecture` → `architecture_code`, added `architecture_name` and `generation`; `gpu_catalog` — added `series`, `board`, `gpu_class` (existing `architecture` column kept for future curation). All six new columns are JSON provenance bundles, user-curated, not auto-seeded. Live DB carries 65 curated CPU rows and 13 GPU rows. The section heading `Boards` was renamed to `Graphics` in the view registry; underlying `products.boards` column unchanged for bridge-layer back-compat. Tests 349 → **364 green**. **Stage 10c (review queue triage redesign) is now next active.**

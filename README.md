...
**Stage 11 (database hierarchy layer) brainstorm complete Session 47 (2026-05-21).** 5-level identity hierarchy locked: `Brand | Sub-brand | Series | Product | Year`. Sub-brand is the marketed-distinct premium gaming line (ROG / Alienware / OMEN-legacy / HyperX-2026+ / Legion / Predator-future); NULL for brand-level lines (TUF, Victus, LOQ, V-series). Series is populated only when the parent has multiple named sub-lines (Strix/Zephyrus/Flow under ROG; Area-51/Aurora under Alienware; Legion 5/7/9 under Legion). Tier modifiers (Pro, Essential, Max, Slim, Transcend, Scar, X) ride on Product, not Series. Echo-parent display rule: empty Sub-brand or Series cells render the closest populated parent in italic-faint. **(Product, Year) becomes the PK** — extends Lenovo's Stage-7 chassis-merge pattern to all brands; underlying SKU codes preserved in `source_model_codes`. **HP HyperX 2026 rebrand handling** baked in: pre-2026 = Sub-brand=OMEN, Series=∅; 2026+ = Sub-brand=HyperX, Series=OMEN. Per-brand audit lock: **77 source rows → 56 product rows** (20 cross-row merges, 19 year corrections). Browse + Compare picker reshape: 3 dropdowns (Brand → Series → Product) + Year toggle button block (multi-select, default 2026) + Status toggle button block (Active/Discontinued, default Active) + unified-sections union view. Compare uses the same per-column. 8-phase implementation plan approved (schema migration → recurate → new picker → union view → echo-parent → Find → tests → docs); ~4–5 sessions. **Stage 10c (review queue triage redesign) + small UI polish brainstorm both queued behind Stage 11.** See `SESSION_LOG.md` Session 47.

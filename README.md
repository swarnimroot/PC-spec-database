...
**Stage 11 (database hierarchy layer) — Phases 1–5 shipped Sessions 48–50 (2026-05-21 → 2026-05-26).** 5-level identity hierarchy locked Session 47: `Brand | Sub-brand | Series | Product | Year`. Sub-brand is the marketed-distinct premium gaming line (ROG / Alienware / OMEN-legacy / HyperX-2026+ / Legion / Predator-future); NULL for brand-level lines (TUF, Victus, LOQ, V-series). Series is populated only when the parent has multiple named sub-lines (Strix/Zephyrus/Flow under ROG; Area-51/Aurora under Alienware; Legion 5/7/9 under Legion). Tier modifiers (Pro, Essential, Max, Slim, Transcend, Scar, X) ride on Product, not Series. Echo-parent display rule (P6, pending): empty Sub-brand or Series cells will render the closest populated parent in italic-faint. **(Product, Year) is the PK** as of Session 48 — extends Lenovo's Stage-7 chassis-merge pattern to all brands; underlying SKU codes preserved in `source_model_codes`. **HP HyperX 2026 rebrand handling** baked in: pre-2026 = Sub-brand=OMEN, Series=∅; 2026+ = Sub-brand=HyperX, Series=OMEN. Per-brand audit lock: **76 source rows → 56 product rows** (20 cross-row merges, 19 year corrections). Browse + Compare picker reshape both shipped: Browse Session 49 (P3), Compare Session 50 (P5) — 3 dropdowns (Brand → Series → Product) + Year toggle button block (multi-select, default 2026) + Status toggle button block (Active/Discontinued, default Active) + union-sections view; Compare uses the same per-column. 8-phase plan: P1 schema swap + P2 recurate (S48), P3 Browse picker + P4 union spec primitive (S49), P5 Compare adopts picker + union grid (S50) — all shipped. P6 echo-parent, P7 Find narrow-by reshape + result-card identity line, P8 tests + docs + status curation — pending. **Stage 10c (review queue triage redesign) + small UI polish brainstorm both queued behind Stage 11.** See `SESSION_LOG.md` Sessions 47–50.

...
| `family_code` | string, nullable | Plain scalar — no bundle. Canonical family identifier for vendors with the multi-SKU Intel/AMD merge: Lenovo (`legion-pro-5-16-gen-10`, Stage 7 T7.0a), ASUS TUF (`asus-tuf-gaming-16-2025`, T9.3), Dell (`da15260x`, S63 — SKU final digit 0 = Intel / 5 = AMD, 0↔5 pair only). Set by the bridge on new ingests and by the per-vendor `backfill-*-families` CLIs on pre-derivation rows; NULL on unmerged rows (HP, ROG, non-pair Dell codes). |

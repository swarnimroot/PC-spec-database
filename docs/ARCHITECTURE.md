...
The transaction logic is exposed as a library function `manual_edit_cell(conn, *, model_code, year, field_path, value, status="vouched", note=None, entered_by=None) -> dict` (T8.6 — Session 32) so the Phase 2 UI (`ui/edit.py`) calls the exact same handler the CLI calls; `main(args)` is a thin argparse wrapper that maps `ValueError → SystemExit("manual-edit: …")`. Caller owns the connection. Returns `{model_code, year, field_path, before, after}` for the post-write confirmation banner the UI renders.
...
- **`views/formatting.py`** — six visible markers (`[verified]`, `[?]`, `[—]`, `[m]`, `[empty]`, `[partial]`), a `marker_for_bundle` mapper, an `aggregate_markers` collapser, a `display_value` stringifier, and a `format_leaf` one-line renderer. Plus `render_scalar_section` for the all-scalar categories. T9.2 added `canonicalize_display(field_path, rendered)` + `_CANONICAL_BY_FIELD` lookup: when callers pass a `field_path`, duplicate-meaning string variants collapse to a single canonical form (currently `panel_type` "IPS-level" → "IPS"). Section views and inline renders pass nothing and see the raw stored string; the find-screen dropdown and cell-match path opt in.
...
└── ui/
    ...
    ├── _markers.py            # shared marker palette + path resolver — T8.4 (rule-of-three extraction)
    ├── hub.py                 # dashboard landing — T8.0 (counts) / T8.2 (4 destinations) / T8.6 (Explore + Curate split) / T8.7 (Curate row full)
    ├── browse.py              # product profile view — T8.1 (Session 41 refresh: Company → Product → Year cascading picker + Section/Feature/Value HTML table; Identity moved out of the body table into a context strip above it)
    ...
    ├── edit.py                # manual-edit a cell — T8.6
    └── refresh.py             # refresh trigger (one / all + live progress) — T8.7
```

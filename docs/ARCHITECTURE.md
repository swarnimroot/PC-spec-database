...
- **Testing strategy.** Action handlers reuse existing `ingest/*` modules, so the pre-UI test suite already covers writes; UI tests focus on rendering + form validation. Streamlit's `AppTest` harness is live as of T8.8 first slice (Session 34) — `tests/ui/test_app_smoke.py` covers entry-point boot + view dispatcher + one read-only screen against an empty-DB fixture; per-screen interaction tests against the 3 write screens are the next cut, with `monkeypatch.setattr("competitive_database.ui.refresh.refresh_product", …)` (UI bound reference, not CLI) the pattern for the network-touching one.

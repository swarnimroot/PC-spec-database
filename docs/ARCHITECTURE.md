...
└── ui/
    ...
    ├── hub.py                 # dashboard landing — T8.0 (counts) / T8.2 (4 destinations) / T8.6 (Explore + Curate split)
...
**CLI ↔ UI write-path crossover (T8.5 + T8.6 — Sessions 31–32).** Phase 2 ends the read-only invariant. The architectural rule "all writes go through helpers; the UI calls those same helpers" (see §Forward compatibility) now binds. Two CLI handlers carry pure-library entry points alongside their argparse `main(args)` shells:

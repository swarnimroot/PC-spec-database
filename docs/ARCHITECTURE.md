...
```json
{
  ...
  "source_url": "https://www.displaymate.com/...",
  "entered_by": "swarnim",
...
`source_url` on manual bundles is optional (added Stage 10a / Session 43) — persisted only when `manual-edit --source-url …` (or the Edit UI form's Source URL field) is set.

**Status values:**
- Scraped: `verified` / `needs-review` / `vendor-doesn't-publish`
- Manual: `vouched` / `needs-review` / `manual` (the third value `manual` was promoted to a real status Stage 10a / Session 43; enforced via `db/helpers._MANUAL_STATUSES`, no schema migration since status is free-text JSON. `views/formatting.marker_for_bundle` checks `status == "manual"` first and returns `MARKER_MANUAL` regardless of `entered_by`.)
...
The transaction logic is exposed as a library function `resolve_row(conn, *, row_id, action, value=…, note=…, entered_by=…) -> dict` (T8.5 — Session 31) so the Phase 2 UI calls the exact same handler the CLI calls; `main(args)` is a thin argparse wrapper that maps `ValueError → SystemExit("resolve: …")`. `coerce_value_string(raw)` is exposed alongside it for callers that want to mirror the CLI's `--value` int → float → string coercion.
...
The transaction logic is exposed as a library function `manual_edit_cell(conn, *, model_code, year, field_path, value, status="vouched", note=None, entered_by=None, source_url=None) -> dict` (T8.6 — Session 32; `source_url` parameter added Stage 10a / Session 43) so the Phase 2 UI (`ui/edit.py`) calls the exact same handler the CLI calls; `main(args)` is a thin argparse wrapper that maps `ValueError → SystemExit("manual-edit: …")`. Caller owns the connection. Returns `{model_code, year, field_path, before, after}` for the post-write confirmation banner the UI renders. `status` accepts `vouched` / `needs-review` / `manual` (the third value was promoted to a real status Stage 10a).
...
├── pyproject.toml
├── .streamlit/
│   └── config.toml             # Stage 10a — light-theme defaults
...
│   │   ├── runner.py
│   │   └── catalog_resolve.py
│   ├── query/                  # S54 — streamlit-free query engine (extracted from ui/find.py)
│   │   └── engine.py
│   ├── api/                    # S54 — FastAPI backend (no Streamlit dependency)
│   │   ├── app.py
│   │   └── serializers.py
│   ├── curation/               # S54 — streamlit-free conflict-queue listing (extracted from ui/triage.py)
│   │   └── queue.py
│   ├── views/
│   │   ├── __init__.py
...
- **`views/formatting.py`** — six visible markers (`[verified]`, `[?]`, `[—]`, `[m]`, `[empty]`, `[partial]`), a `marker_for_bundle` mapper, an `aggregate_markers` collapser, a `display_value` stringifier, and a `format_leaf` one-line renderer. Plus `render_scalar_section` for the all-scalar categories. T9.2 added `canonicalize_display(field_path, rendered)` + `_CANONICAL_BY_FIELD` lookup: when callers pass a `field_path`, duplicate-meaning string variants collapse to a single canonical form (currently `panel_type` "IPS-level" → "IPS"). Section views and inline renders pass nothing and see the raw stored string; the find-screen dropdown, the cell-match path, and the Compare detail accordion (`api/serializers.py::_detail_rows`, Session 60) opt in. Stage 10a (Session 43) added `field_type(path)` + `coerce_to_field_type(path, raw)` so the Edit UI can drop its JSON-literal toggle and infer per-field types behind the scenes (suffix rules for numerics; explicit override map for booleans / specific ints / floats; default str). Same module also gained a `marker_for_bundle` arm that returns `MARKER_MANUAL` when `status == "manual"` (Stage 10a — `manual` promoted to a real third bundle status).
...
### Stage 10b — Section rollup display rule (Sessions 44 + 46)

Browse / Compare / Find collapse each visible section to a single rollup line (I/O is the one multi-row exception — four sub-rows). Per-SKU detail is preserved in the DB row and stays visible on the Edit screen — the rollup is a render-time pivot, not a stored value.

**Batch 1 — CPU + Graphics (Session 44):**
...
- **Section heading rename:** `Boards` → `Graphics` in `views/orchestrator._SECTION_REGISTRY`, `ui/_components.friendly_leaf_label`, and the `views/boards.render` heading. The underlying `products.boards` column name is unchanged — the rename is presentation-only, so bridge / merge / DB code carries through. Session 46 also renamed the CPU heading `CPU` → `Processor` along the same lines.
- **Render-time enrichment:** `_components.spec_table_html` and `_components.comparison_grid_html` accept optional `cpu_catalog` and `gpu_catalog` kwargs; `ui/spec_roster.py` (the merged Browse + Compare screen) and `ui/find.py` load the catalogs at the same point they load each product and pass through.

**Batch 2 — remaining 14 sections (Session 46):**

- Every other visible section's `views/<name>.py` exposes `rollup_value(product) -> (str, marker)` returning one collapsed line. Rollups for batch-2 sections don't need catalog joins — they consume the product dict directly.
- **I/O is the one multi-row exception.** `views/io.py` exposes `rollup_rows(product) -> list[(label, value, marker)]` returning four sub-rows (USB / HDMI / SD card / audio jack), each with its own worst-status marker. `_components.py` carries `_rollup_rows_html` / `_cmp_rollup_rows_html` helpers for this case alongside the single-row helpers used by the other sections.
- **`_components._rollup_for_section` dispatch table** wires every visible section's `rollup_value` / `rollup_rows` into `spec_table_html` + `comparison_grid_html`.
- **Edit screen path preservation:** every section's `field_paths()` is unchanged. The per-SKU / per-offering bundle paths remain fillable through `manual-edit` / the Edit UI for every section.

**Visible-vs-walked split (Session 46):**

The section registry now has two parallel concepts:

- **`views/orchestrator._SECTION_REGISTRY`** — the full ordered list of every section. Edit + `find-empty` + every CLI walker iterate this. Unchanged shape; still the canonical "every spec category in the DB" list.
- **`views/orchestrator._VISUAL_SECTIONS`** — a tuple naming the sections that contribute a row (or sub-rows) to the visual rollup tables on Browse / Compare / Find. Excludes Keyboard (verbose free-text descriptions that resist clean rollup) and Thermals (100% NULL across the DB today). Both excluded sections remain editable and still surface in `find-empty`.

The split is purely presentational — no data is hidden from the DB, the bridge layer, or any write path. A section can be made visible later by adding its name to `_VISUAL_SECTIONS` and shipping a `rollup_value` / `rollup_rows` on the corresponding `views/<name>.py`.
...
| GET | `/api/model/{id}/detail?year=` | raw per-leaf detail values grouped by Compare section (`year` required; 422 if missing) — actual CPU/GPU model names, display specs, raw ports (multi-offering leaves as lists); powers the Compare accordion on Processor/Graphics/I/O (Session 58) |
...
├── .streamlit/                # Stage 10a — Streamlit light-theme config
│   └── config.toml
└── ui/
    ├── __init__.py
    ...
    ├── hub.py                 # dashboard landing — Stage 10a rewrite: hero line + 3 metric tiles + rounded CTA cards; S54 UI rework: Spec Roster + Find CTAs (Browse / Compare cards merged), pill switcher renders Spec Roster / Find inline
    ├── spec_roster.py         # view one product or compare several (S54 UI rework — Browse + Compare merged): single `Brand · Series · Product` search combobox per column + Year/Status toggle blocks; one populated column → union identity strip + `union_spec_table_html`; two or more → multi-column `comparison_union_grid_html`. Carries forward the Stage 11 Phase 3 (S49) / Phase 5 (S50) picker + union primitives from the former browse.py / compare.py
    ├── find.py                # filter playground — Stage 10a rewrite: three-box query bar (Spec field / Match / Value) with plain-English ops; rounded result cards with `Open →` cross-nav; Stage 11 Phase 7 (S52) reshaped the narrow-by block onto the new `brand_series_product_picker` + `year_toggle_block` + `status_toggle_block` components (partial picks narrow; Brand-only is a valid narrow — not a strict cascade); legacy 3-selectbox shape (Company / Series / Year with `(any)` sentinel) removed
    ...
    ├── edit.py                # manual-edit a cell — Stage 10a rewrite: bulk per-product editor (cascading picker → all fields vertical with `✎` inline expand → bottom Save bar with live change count); JSON-literal toggle dropped
    └── refresh.py             # refresh trigger — Stage 10a rewrite: three-mode tabs (All products / By company / Selected products); slim in-progress card + 4-tile post-run result panel; Custom-URL mode dropped
```
...
`ui/_markers.py` (T8.4 — Session 30) hosts the shared rendering primitives that survived past the rule-of-three threshold: `MARKER_COLORS` (six-marker palette → hex), `colorize_marker` / `colorize_text` (one-token + bulk-substitution span wrappers), `resolve_path(product, path)` (dotted-path → `(bundle, plain)` resolver covering both bundle-shape leaves and the plain-string `boards.N.arch_marker` leaf), and `render_cell(bundle, plain)` (one-cell `value [marker]` HTML). `ui/spec_roster.py` (the merged Browse + Compare screen), `ui/find.py`, `ui/triage.py`, and `ui/edit.py` all import from this module; nothing else does. Promotion trigger: `ui/find.py` (T8.4) became the third caller, matching the convergence point flagged in the T8.3 inline note.

**CLI ↔ UI write-path crossover (T8.5 + T8.6 + T8.7 — Sessions 31–33).** Phase 2 ends the read-only invariant. The architectural rule "all writes go through helpers; the UI calls those same helpers" (see §Forward compatibility) now binds. Three CLI handlers carry pure-library entry points alongside their argparse `main(args)` shells:

- `cli/resolve.py` exposes `resolve_row(conn, *, row_id, action, value=…, note=…, entered_by=…) -> dict` — used by `ui/triage.py`.
- `cli/manual_edit.py` exposes `manual_edit_cell(conn, *, model_code, year, field_path, value, status, note, entered_by) -> dict` — used by `ui/edit.py`.
- `cli/refresh.py` exposes `refresh_product(conn, *, brand, model=…, url=…, from_db=…, year=…, profiles_dir=…, on_step=…) -> dict` and `refresh_all_products(conn, *, profiles_dir=…, on_step=…) -> dict` — used by `ui/refresh.py`.

All three families share the same shape: caller owns the connection (no open/close inside), wrap their writes in a transaction, raise `ValueError` on user-fixable errors (the CLI wrapper maps `ValueError → SystemExit("<cli-name>: …")`), and return a summary dict. `coerce_value_string(raw)` is exposed from `cli/resolve.py` and re-imported by `ui/edit.py` so the manual-edit text input coerces user input identically to the CLI's `--value` flag (int → float → string). The `refresh_*` pair adds one extra wrinkle: an optional `on_step` callback that fires at every phase transition (`resolve_url` / `fetch start|done` / `dispatch start|done` / per-PK `ingest start|done` / `complete`; bulk mode adds `plan` / `skip` / `product_start|done|error`) so callers can stream live progress to the user. The CLI uses it to print per-tile lines; `ui/refresh.py` uses it to pump a live log into `st.status`.

T8.0 (Session 26) shipped `ui/__init__.py`, `ui/app.py`, `ui/hub.py`, and `cli/ui_launch.py`. T8.1 (Session 27) added `ui/browse.py` and wired hub → browse navigation through `st.session_state["view"]` (a single string key dispatched by `app.py`); the same key is the planned hand-off point for T8.2's four-destination grid and T8.3–T8.5's per-screen entries. T8.2 (Session 28) replaced the single hub button with a four-destination grid and added the days-since-refresh metric; `ui/compare.py`, `ui/find.py`, `ui/triage.py` shipped as placeholder stubs ahead of T8.3–T8.5 (the queue-triage module is filenamed `triage.py` to avoid shadowing the stdlib `queue` module that Streamlit's path watcher imports; the dispatch route key in `st.session_state["view"]` stays `"queue"`). T8.3 (Session 29) replaced the `ui/compare.py` stub with the side-by-side grid: vendor / segment / status selectboxes (vendor backed by the `brand` column, not `vendor_full_name` — the OEM is the conceptual filter granularity), `st.multiselect` over the filtered `(model_code, year)` pool, then an HTML table whose row registry is the union of `views.orchestrator.all_field_paths` across selected products (section + first-appearance order preserved), rendered with section group headers and the six provenance markers wrapped in color spans. Path resolution handles both bundle-shape leaves and the plain-string `boards.N.arch_marker` leaf. T8.4 (Session 30) replaced the `ui/find.py` stub with the single-field filter playground: section → field cascade over the union of `all_field_paths` with offering indices collapsed to a `*` wildcard (so the user picks one slot and the filter sweeps every instantiated offering on every product), comparator dropdown (`= / contains / ≥ / ≤ / is set / is empty / vendor doesn't publish`), value text input shown only when the comparator needs one, per-product result blocks listing every matching cell with the same colored marker HTML as the browse + compare screens. At the same time, the marker color map + `_resolve_path` + cell-render helpers shipped in T8.1 / T8.3 were lifted into `ui/_markers.py` (see preceding paragraph) and `ui/browse.py` + `ui/compare.py` were refactored to import from there. T8.5 (Session 31) replaced the `ui/triage.py` stub with the review-queue triage screen — sidebar list of unresolved rows (compact one-line summary) + main-panel diff (existing | candidate side-by-side, with provenance markers, source URLs, timestamps, and per-key captions). Action buttons match the `resolve` CLI verbs; `new_chip_unverified` rows show only `accept_candidate` / `dropped` (the other two are rejected at the handler level — UI hides them with a caption pointing at the policy). Manual-override is gated behind an `st.expander` with a "treat value as JSON literal" checkbox so booleans / null / lists round-trip cleanly. The connection from `ui/app.py` is passed straight through to `resolve_row` — same write path the CLI uses. Resolve outcomes survive the post-action `st.rerun()` via two `st.session_state` keys (`queue_last_resolved` and `queue_last_error`) so the user sees the confirmation banner on the refreshed page. T8.6 (Session 32) added `ui/edit.py` — manual-edit a cell as the second UI write path. Form layout: product selectbox → section / field cascade (same union-of-templates pattern T8.4 uses, offering indices collapsed to `*`) → offering-index selectbox (appears only when the chosen template is wildcarded; lists `range(len(offerings))` for the selected product) → current-value preview line (rendered with the shared provenance marker) → submit form (value text input + JSON-literal checkbox + status radio + note + entered-by override). Hub split into two rows at the same beat: "Explore" carries the three read-only destinations (browse / compare / find); "Curate" carries the two write destinations (review-queue triage / manual-edit) with the third slot intentionally reserved for T8.7's refresh trigger. Post-save confirmation survives `st.rerun()` via `st.session_state["edit_last_summary"]` (before / after rendered side-by-side using `render_cell`) and `st.session_state["edit_last_error"]` for the ValueError surface. T8.7 (Session 33) added `ui/refresh.py` — refresh trigger as the third UI write path; fills the Curate row's reserved third slot. Top-level mode radio (`One product` / `All products`). One-product branch carries a sub-radio for URL source (URL template / Custom URL / From DB stored source_url) over brand + model + URL inputs; the URL template option renders a live URL preview caption as the user types. The "From DB" picker reads `_enumerate_refresh_targets(conn)` (planning helper that mirrors the loop inside `refresh_all_products`) and lists eligible products with their stored-URL count (Lenovo Intel+AMD rows show as "2 URLs"). All-products branch renders three metric tiles (Eligible / Skipped / Total) + a will-refresh list + a collapsible skipped expander + a single "Refresh all N" button. Submit on either branch wraps the lib call in `with st.status("Refreshing…", expanded=True)` and pumps the live log via the `on_step` callback (one `st.write` line per surfaced event; `status.update(label=…)` flips on fetch / dispatch / ingest / per-product-start; final `state="complete"` or `state="error"`). Post-run summary survives the rerun via `st.session_state["refresh_last_summary"]` (mode-tagged) and `["refresh_last_error"]`. The CLI's `--all` flag, previously a "not implemented yet" SystemExit, now wires through to `refresh_all_products` — same skip/error rules as the UI's All-products mode. T8.8 first slice (Session 34) added the README "Launching the UI" Quick-start subsection inside §Setup; a Custom-URL host pre-flight check in `ui/refresh.py` (`_BRAND_HOST_SUFFIXES` map + `_validate_url_brand` helper) that rejects brand/URL mismatches before the request leaves the UI; and the first `streamlit.testing.v1.AppTest` smoke harness under `tests/ui/` (`conftest.py` with the `empty_db` fixture that injects `COMPETITIVE_DB_PATH`; `test_app_smoke.py` with three smoke tests over the entry-point boot + view dispatcher + one read-only screen). T8.8 (b-expand) (Session 35) extended the harness with per-screen write-path interaction tests against all three write screens — `test_triage_apptest.py` (seeded `value_disagreement` queue row + Accept-candidate click + DB write-through assertion), `test_edit_apptest.py` (single-product seed + default Identity / `vendor_full_name` cascade + Save-manual-edit click + manual-bundle assertion), `test_refresh_apptest.py` (URL-template mode + monkeypatched `ui_refresh.refresh_product` + summary-banner assertion; second test stubs `ValueError` to assert the `_run_one` try/except → session_state → `st.error` pipeline). T8.8 closes here; (c) refresh-targets enumeration dedup (`ui/refresh.py:104-152` ↔ `cli/refresh.py:334-374`) and (d) cross-product URL dedup in `--all` (`cli/refresh.py:709-715`'s `_collect_source_urls_from_product`) move to TASKS §Deferred under their existing gates (rule-of-three / shared-URL-pattern). 314/314 tests green; Stage 8 closed. **T8.1 refresh (Session 41)** — `ui/browse.py` rewritten end-to-end to replace the colored-marker orchestrator-text dump with a three-level cascading picker (Company → Product → Year; brand-grouped products, year-stripped `vendor_full_name` as the Product label with first-comma trim for HP's marketing-string case) and a Section / Feature / Value HTML table with rowspan-merged Section cells. Identity rows moved out of the body table into a context strip above it (sub-brand / series / status / segment); vendor + brand + year are folded into the dropdowns themselves. Empty sections (e.g. CPU when no CPU data scraped) render as a single "(no data scraped)" row instead of being skipped, keeping the schema shape visible. Marker palette reused from `ui/_markers.py` verbatim — no new color logic. `cli/ui_launch.py` gained `--base-path` for reverse-proxy hosting (auto-applies `--server.baseUrlPath` + `--server.headless=true` + CORS/XSRF off). 339/339 tests still green.

**Stage 10a — Visual polish (Session 43, 2026-05-19).** Eight-phase Streamlit redesign under one editorial-spec-sheet visual identity. Foundation (Phase A): design tokens centralized in new `competitive_database/ui/theme.py` (`PALETTE` / `SPACE` / `RADIUS` / `TYPE` + `inject_global_css()`); shared chrome in new `competitive_database/ui/_chrome.py` (`render_header(active)` top-bar nav with Browse / Compare / Find + `···` overflow, `render_footer()` thin divider); expanded `competitive_database/ui/_components.py` for cross-screen helpers (`cascading_picker`, `identity_strip_html`, `spec_table_html`, `comparison_grid_html`, `marker_legend_inline_html`, `dot_marker`, `MARKER_LABELS`, `friendly_field_label`, `friendly_leaf_label`); `.streamlit/config.toml` for light-theme defaults. Per-screen rewrites: Hub (B) → hero line + 3 metric tiles + 3 rounded CTA cards with `Open →` primary buttons (the Review-queue metric was dropped — triage parked through 10a). Browse (C) → ~20-line shell delegating to `_components`; light identity strip; colored `●` dot markers replace bracketed text tokens; inline marker legend at table header. Compare (D) → N vertical picker columns (1–4 max) with `+` / `×`; segment auto-shown read-only beneath each pick; `comparison_grid_html` renders a strict-majority (`max_count * 2 > N`) `▌` left-edge divergence cue. Find (E) → three-box query bar (Spec field / Match / Value) over plain-English labels (50-entry `_LEAF_LABEL_OVERRIDES` map; 7 ops in English, no math symbols); optional Company / Year narrow-by; rounded result cards with `Open →` cross-nav to Browse. Edit (F) → bulk per-product editor (cascading picker → all spec fields listed vertically with dotted section headers → compact `feature · ● value · ✎` rows → inline expand form → bottom Save bar with live change count + disabled-at-zero state). The JSON-literal toggle was dropped entirely; type coercion is inferred from the field path via new `views/formatting.py::field_type(path)` + `coerce_to_field_type(path, raw)` helpers (suffix rules: `_wh/_mm/_kg/_ms` → float; `_mhz/_hz/_w/_gen/_count/_pct/_cores/_gb/_mb/_mts` → int; explicit override map for booleans + a few specific ints/floats; default str). Refresh (G) → three-mode tabs (All products / By company / Selected products; Custom-URL mode dropped along with the 16 obsolete `tests/ui/test_refresh_helpers.py` cases); slim `cd-refresh__progress-card` (`Refreshing K of N…` + `st.progress`) replaces the streaming `st.status` event log; post-run result panel persists in session state with 4 metric tiles + "What needs attention" + Skipped expander. `cli/refresh.py` lifted `_VENDOR_TEMPLATES` → `VENDOR_TEMPLATES` and `_collect_source_urls_from_product` → `collect_source_urls_from_product` to public names (back-compat aliases retained). Phase H cleanup deleted dead helpers in `_markers.py` (`colorize_text`, `colorize_marker`, `render_cell`), the `.cd-footer` CSS class, and chrome-only artifacts on `triage.py` (`← Hub` button + `T8.X` captions); the full triage redesign is parked for Stage 10c. Two data-model extensions delivered mid-stream: (1) `manual` joined `vouched` and `needs-review` as a real third bundle status — enforced Python-side via `db/helpers._MANUAL_STATUSES`, with `views/formatting.marker_for_bundle` returning `MARKER_MANUAL` (blue dot) when `status == "manual"` regardless of `entered_by`; (2) `manual-edit` (CLI + UI) now persists a top-level `source_url` on the manual provenance bundle (new `--source-url` CLI flag; Edit UI form's Source URL field). Tests 339 → 349 (peaked 364; settled 349 after Phase G prune).

**Stage 10b — Data display rollup (Sessions 44 + 46).** FULLY CLOSED 2026-05-20 across two batches. Display-rule + visible/walked split documented above under §Stage 10b — Section rollup display rule. Batch 1 (S44) introduced the pattern with CPU + Graphics rollups, the schema migration for the new curated catalog columns, and the `Boards` → `Graphics` heading rename. Batch 2 (S46) extended the pattern to the remaining 14 sections, hid Keyboard + Thermals from the visual rollup via the new `_VISUAL_SECTIONS` tuple, introduced the multi-row I/O case (`rollup_rows` returning four sub-rows), renamed the CPU heading `CPU` → `Processor`, and redesigned Browse / Compare / Find on top of the new rollup tables (Series rung replaces Product rung, strict cascade, Compare `+` button on a faint rail, Find result cards with `Open →` cross-nav to Browse pre-loaded). `_components.py` grew screen-helper additions to drive the redesigns: `cascading_picker` gained `strict_cascade: bool` + `rung_mode: "product" | "series"` kwargs; new private `_series_cascade` parallel to `_product_cascade`; new public `inject_compare_styles`, `find_rollup_for_section`, `find_result_card_html`, `inject_findcard_styles`. Tests 349 → 364 (S44) → 479 (S46). Stage 10c (review queue triage redesign) is next active, gated on a small UI polish brainstorm queued for Session 47.

**Stage 11 — Database hierarchy layer (Sessions 48–52, 2026-05-21 → 2026-05-26).** Picker pattern + union spec rendering rebuilt on top of the new `(product, year)` PK. New picker is a strict 3-rung Brand → Series → Product cascade (`brand_series_product_picker`) over the curated `product` column, paired with two toggle blocks beneath it — `year_toggle_block` (multi-select year pills, default latest) and `status_toggle_block` (Active / Discontinued pills, default Active; NULL status treated as Active). Union spec rendering is centralized in `_union_rollup_for_section` (the Policy B ` · ` middle-dot join primitive) plus `union_spec_table_html` (Browse, N-row single-column) and `comparison_union_grid_html` (Compare, N-column multi-row); `union_identity_strip_html` carries the corresponding identity strip. Browse rewrite (P3 — Session 49) replaces the Series-rung cascade with the 3-rung picker + toggle blocks + union spec view; N=1 is byte-identical to the legacy single-product renderer. Compare rewrite (P5 — Session 50) gives every column the same per-column 3-rung picker and renders the multi-column union grid in place of the legacy single-product Compare grid. P4 (Session 49) extracted the union spec table primitive itself, used today on Browse and adopted by Compare in P5. P6 (Session 51) added the echo-parent display rule: when a product's `sub_brand` is NULL, the nearest non-null ancestor (brand) renders in italic-faint at the sub_brand slot; when `series` is NULL, the nearest non-null ancestor (sub_brand if populated, else brand) renders the same way. The rule is implemented as one private helper — `_echo_parent_for_leaf(key, product) -> tuple[str | None, bool]` — threaded into `identity_strip_html`, `union_identity_strip_html`, `_union_column_header`, and `find_result_card_html`. Two CSS classes drive the italic-faint render: `cd-identity__value--echo` (Browse + Compare strips) and `cd-findcard__crumb--echo` (Find result cards); both consume the existing `--cd-text-faint` token. Two locked behaviors: union strips with some populated + some NULL at the same rung render the real value plain (echo only fills total-absence, never appears alongside a real value); Find result cards always emit three identity crumbs (brand + sub_brand-or-echo + series-or-echo) + year regardless of which rungs are populated on the underlying row. Pure rendering layer — no schema, no data, no CLI surface changes. P7 (Session 52) reshaped Find's narrow-by block onto the same Phase 3 components: `_render_narrow_by` in `ui/find.py` now consumes `brand_series_product_picker(conn, key_prefix="find", strict=True)` + `year_toggle_block` (scoped to products consistent with the upstream Section/Feature/Match/Value query) + `status_toggle_block` (default empty — no filter, unlike Browse/Compare's Active default). Partial picks narrow: Brand-only is a valid narrow (locked user decision — strict cascade is intentionally rejected here because Find's goal is "narrow the result set," not "resolve to one product"). Legacy session keys (`find.narrow.company` / `.series` / `.year`) and the `_ANY = "(any)"` sentinel were removed. P8 (Session 52) landed the tests slice (+4 new, 2 fixed) + this docs alignment pass + the status curation pass — all 56 products now carry a manual `status` bundle (`status="vouched"`, `entered_by="session52-curation"`) via a one-shot year rule applied through `scripts/curate_status_session52.py`: 2025/2026 → Active (24 + 20 = 44), 2023/2024 → Discontinued (3 + 9 = 12). Rule is intentionally rough; spot-checks for older-year products still on sale are the follow-up.
...
- **Refresh progress streaming.** Locked at T8.7 (Session 33). Approach: `with st.status("Refreshing…", expanded=True)` block + an `on_step` callback the lib fns fire at every phase transition. Inside the block, `st.write(line)` appends one log line per event; `status.update(label=…)` advances the spinner label; `status.update(state="complete"|"error")` switches the framework icon. No SSE channel, no polling — the click-rerun handles everything synchronously. CLI reuses the same callback contract for its print lines.
- **Testing strategy.** Action handlers reuse existing `ingest/*` modules, so the pre-UI test suite already covers writes; UI tests focus on rendering + form validation. Streamlit's `AppTest` harness is live as of T8.8 first slice (Session 34) — `tests/ui/test_app_smoke.py` covers entry-point boot + view dispatcher + one read-only screen against an empty-DB fixture. T8.8 (b-expand) (Session 35) added per-screen write-path interaction tests over all three write screens — `tests/ui/test_triage_apptest.py` (seeded `value_disagreement` queue row + Accept-candidate click + DB write-through assertion), `tests/ui/test_edit_apptest.py` (single-product seed + default Identity / `vendor_full_name` cascade + Save-manual-edit click + manual-bundle assertion), `tests/ui/test_refresh_apptest.py` (URL-template mode + monkeypatched `ui_refresh.refresh_product` + summary-banner assertion; second test stubs `ValueError` to assert the `_run_one` try/except → session_state → `st.error` pipeline). The network-touching refresh test uses `monkeypatch.setattr` on the UI module's bound `refresh_product` reference (not the CLI module's — `from … import` rebinds at import time, and `_run_one` resolves the symbol against its module's globals at call time).

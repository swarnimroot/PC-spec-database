...
- Stage 8 / Phase 2 UI — scope locked Session 25, T8.0 shipped Session 26, T8.1 shipped Session 27, T8.2 shipped Session 28, T8.3 shipped Session 29, T8.4 shipped Session 30, T8.5 shipped Session 31, T8.6 shipped Session 32, T8.7 shipped Session 33, T8.8 first slice shipped Session 34; see [`PRD.md` §Phase 2](PRD.md#phase-2--ui-stage-8) + [`ARCHITECTURE.md` §UI layer](ARCHITECTURE.md#ui-layer-phase-2)
  ...
  - T8.7 — Refresh trigger — one-product / all modes over URL-source radio (template / custom URL / from-DB); `cli/refresh.py` exposes `refresh_product` + `refresh_all_products` lib fns with `on_step` streaming; CLI `--all` flipped from "not implemented" to live — **done Session 33**
  - T8.8 — Polish — README / launch instructions, smoke tests, anything that surfaces during use — **first slice Session 34** (README "Launching the UI" Quick-start subsection + Custom-URL host pre-flight check in `ui/refresh.py` + `tests/ui/` seeded with 16 helper tests + first AppTest smoke layer with 3 tests over entry-point / dispatcher / one read-only screen; **310/310 green**). Remaining: (a) per-screen AppTest interaction tests against the 3 write screens (triage row-resolve, edit cell-save, refresh with monkeypatched `refresh_product`); (b) `_enumerate_refresh_targets` ↔ `refresh_all_products` planning loop dedup — gated on third caller surfacing per Session 33 Decision #8; (c) `_collect_source_urls_from_product` cross-product URL dedup in `--all` mode — noted no-op until shared URL pattern emerges; (d) any rough edges that surface during use.

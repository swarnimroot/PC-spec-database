...
- Compare per-leaf detail drill-down — new read-only `GET /api/model/{id}/detail?year=` (year required; per-leaf values grouped by Compare section, one row per leaf, offerings `·`-joined + synthetic Graphics GPU row) + `GET /api/schema/detail`; `serializers.model_detail`/`detail_schema` + `_FRIENDLY_LEAF`; Compare sections → collapsible accordions (collapsed = rollup summary, expand fetches real values, cached per model·year) + ☰ Fields drawer (toggle fields/sections) + `api.js` `getModelDetail`/`getDetailSchema`; tests 499 → 505 — DONE S58

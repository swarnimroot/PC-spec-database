...
## Next
- React rebuild — replace Streamlit `ui/` with React frontend + FastAPI backend (`docs/REACT_REBUILD_PLAN.md`, approved 2026-06-08) — **Phase 1 (query engine extracted to `competitive_database/query/`) + Phase 2 (FastAPI `competitive_database/api/` schema/catalog/find endpoints + `frontend/` Vite+React Spec Finder, proven on real data: 37 models, facets + advanced query) DONE S54**; remaining screens (Spec Roster / Edit / Refresh / Triage) + cutover pending; 549 tests green — Session 54

...
## Next
- React rebuild — replace Streamlit `ui/` with React frontend + FastAPI backend (`docs/REACT_REBUILD_PLAN.md`, approved 2026-06-08) — **Phase 1 (query engine extracted to `competitive_database/query/`) + Phase 2 (FastAPI `competitive_database/api/` schema/catalog/find endpoints + `frontend/` Vite+React Spec Finder, proven on real data: 37 models, facets + advanced query) DONE S54**; **React Compare Matrix screen (Spec Roster) DONE S54** (rows = our sections, up to 4 columns via drag-shelf + typeahead, per-column year scrubber, differences-only toggle, reuses shared StateDot/ValueCell; Finder↔Compare nav); **React Curation Cockpit DONE S54** (unifies Edit + Triage — unified queue spanning conflicts 229 + field-state review/missing/hand, 3-pane workbench, adaptive editor resolve-vs-edit, keyboard commit, responsive provenance slide-over; backend write/queue endpoints on `api/app.py` + conflict queue extracted from `ui/triage.py` to new `competitive_database/curation/`; real DB safe, tests on copy); remaining screens (Refresh) + cutover pending; 561 tests green — Session 54
- UI rework — Spec Roster — **Browse + Compare merged into single `ui/spec_roster.py` screen DONE S54** (browse.py/compare.py + their tests deleted; spec_roster.py + test_spec_roster.py added; 3 mockups under docs/) + **Find redesign DONE S54** (one-criterion polish kept; "Open top 4" → checkbox-pick "Compare selected (N) →" with 4-cap disable+hint; per-row Open kept; 516 tests green); remaining rework screens (Edit / Refresh / Triage) pending — Session 54

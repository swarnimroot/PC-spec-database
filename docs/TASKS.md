...
- React Curation Cockpit (`frontend/src/curation/`) — unified queue (conflicts + field-state review/missing/hand) + adaptive editor (resolve-vs-edit) + keyboard commit + provenance slide-over; curation backend on `api/app.py` + `competitive_database/curation/queue.py` — DONE S54
...
- React Refresh screen — wire the React UI to `POST /api/refresh` — OPEN
- React Home (lightened) + once-ever welcome modal (localStorage) — OPEN
- React Phase 4 cutover — retire Streamlit `ui/` + its AppTests once parity reached; grow API/frontend tests — OPEN
- Parked hiccup — Compare empty-state bug: removing all columns blanks the area + breaks drag/select of a first product — OPEN (revisit in baseline-complete brainstorm)
- Parked hiccup — Finder repeated row-label names ("V V16", "Strix Strix G16", "Area-51 Area-51 16"), likely series+model duplicated — OPEN
- Parked hiccup — Curation rename (simpler) + left queue flat-list readability: needs groupings + field-importance prioritization — OPEN
